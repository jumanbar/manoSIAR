...
```{r externos, include=FALSE, cache=FALSE}
# Es decir, no incluir comandos del tipo:
#
# > install.packages(...)
#
...
# a aparecer en los ejes de los gráficos. En caso de no usar 
...
- NT y todos los demás en mg/L, en general están en mg/L  
...
# 
d$periodo <- as.factor(str_to_title(d$periodo))

# Para agregar el parámetro NH3L, a la tabla que está en formato "largo", hace
# falta hacer algunos trucos:
# 
# NOTA: HACER FUNCIÓN PARA EL CÁLCULO DE NH3!
tmp <- 
  # 1. Tomar Temperatura, pH y NAmoniacal y convertir la tabla a formato ancho:
  d %>% 
  dplyr::filter(id_parametro %in% c(2032L, 2018L, 2090L)) %>% 
  mutate(param = case_when(
    id_parametro == 2032L ~ "Temp",
    id_parametro == 2018L ~ "pH",
                     TRUE ~ "NH4"
  )) %>% # Esto facilita escribir la ecuación de abajo:
  ancho %>% 
  # 2. Calcular NH3L:
  mutate(
    # El nombre "valor" facilita el siguiente paso (bind_rows):
    valor = amoniaco_libre(NH4, pH, Temp),
  ...
  select(-starts_with("pH"), -starts_with("Tem"), -starts_with("NH4"))

# Combinar d y tmp:
d <- bind_rows(d, tmp) %>% arrange(id_muestra, id_parametro)
```
...
```{r Olimar_datos, include=FALSE, cache=FALSE, fig.cap="Fisicoquímicos, Río Olimar"}
dx <- dplyr::filter(d, nombre_subcuenca_informes == "Olimar")
```

##### Fisicoquímico

```{r Olimar_fq, cache=FALSE}
g_mes_all(dx, ids_fq, ncol = 3)
```

##### Nutrientes

```{r Olimar_nut, cache=FALSE}
g_mes_all(dx, ids_nut, ncol = 3)
```
...
```{r Olimar_est, cache=FALSE}
g_comp_est_all(dx, ids_comp, ncol = 2)
```
...
g_comp_est_all(dx, id_parametro = ids_comp, ncol = 2)
```
...
c1 <- g_comp_cue(d, id_param = 2009L)
c2 <- g_comp_cue(d, id_param = 2028L)
c3 <- g_comp_cue(d, id_param = 2098L)
c4 <- g_comp_cue(d, id_param = 2101L)
...
```{r cloa, cache=FALSE}
# d <-d %>% filter(!is.na(SST_mg)) %>% 
#   mutate(SST_t = gsub('<LD', '3.1', gsub('<LC', '4.6',SST_mg))) %>% 
#   mutate_at(.vars='SST_t', as.numeric)

d %>% 
  filter(id_parametro == 2000L) %>% 
  ggplot() +
...
dfiet <-
  ...
  dplyr::filter(id_parametro == 2098L & codigo_pto %in% estaciones_loticas) %>% 
  select(id_muestra, nombre_subcuenca_informes, 
         codigo_pto, anio, mes, valor) %>%
  group_by(codigo_pto) %>% # Para agregar por estación
